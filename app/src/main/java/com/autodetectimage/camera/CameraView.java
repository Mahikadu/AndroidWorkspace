		...
		FindDocCornersThread.FindDocCornersListener
{
	...
	public interface Callback {
		...
		void onDetectCorners(CameraView inst, PointF[] points, int failure, float failedRate, float areaAspect);
	}
	...
	public void onAttachedToWindow() {
		...
		docCornersThread = new FindDocCornersThread(getContext());
	...
	*/
	...
	public void documentCornersFound(FindDocCornersThread.Task task) {
		if (task.getThread() == docCornersThread) {
			...
			final Corners bounds = task.documentBounds;
			...
			if (mShowDocumentCorners) {
				...
				PointF [] box;
				if (bounds != null) {
					final float [] boundsPoints = new float[] {
							bounds.points[0].x, bounds.points[0].y,
							bounds.points[1].x, bounds.points[1].y,
							bounds.points[2].x, bounds.points[2].y,
							bounds.points[3].x, bounds.points[3].y
					};
					previewTransform.mapPoints(boundsPoints);
					box = new PointF[] {
							new PointF(boundsPoints[0], boundsPoints[1]),
							new PointF(boundsPoints[2], boundsPoints[3]),
							new PointF(boundsPoints[4], boundsPoints[5]),
							new PointF(boundsPoints[6], boundsPoints[7]),
					};
				} else {
					box = null;
				}
				...
				if (cameraOverlay != null) {
					...
					cameraOverlay.setDocumentBounds(box);
				}

				if (mCallback != null) {
					mCallback.onDetectCorners(this, points, task.cornerFail, task.failedRate, task.areaAspect);
				}
			}
