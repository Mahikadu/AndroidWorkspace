...
import com.autodetectimage.RuntimePermissions;
import com.autodetectimage.SdkFactory;
import com.autodetectimage.console.ConsoleView;
import com.autodetectimage.util.Utils;
import com.autodetectimage.widget.ImageCheckBox;
...
public class CameraActivity extends AppCompatActivity implements MotionDetector.MotionDetectorCallbacks, CameraView.Callback
{
	 ...
	 */
	public static final String PICTURE_SINK = "picture-sink";
	                               ...
	                               boolean forceSingleShot)
	{
		...
		intent.putExtra(PICTURE_SINK, fileSink);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPictureSink = new File(in.getStringExtra(PICTURE_SINK));
		...
		// Open Camera
        final CropDemoApp theApp = (CropDemoApp) getApplication();
		theApp.RuntimePermission.runWithPermission(this, Manifest.permission.CAMERA,
	...
	private boolean allowShot()	{
		return mCameraView.isCameraReady() && cameraStable();
	}
	...
	public void finish() {
		...
		// Notify application is any pictures
		if (mShotCounter > 0) {
			int code;
			Intent data = new Intent();
			if (mPictureUris.size() == 1) {
				// For single picture only
				code = RESULT_OK;
				data.setData(mPictureUris.get(0));
			} else {
				code = RESULT_FIRST_USER + mPictureUris.size();
			}

			// Anyway!
			data.putParcelableArrayListExtra(PICTURES_LIST, mPictureUris);

			// Returns shot count as result
			setResult(code, data);
		} else {
			// No pictures
	...
	public void onPictureReady(final CameraView view, final byte[] pictureBuffer) {
        ...
        CropDemoApp theApp = (CropDemoApp) getApplication();
					...
					public void permissionRun(String permission, boolean granted) {
						if (granted) {
							...
							try {
								...
								pictureReady(pictureFile, true, sequenceCounter);
	...
	private void pictureReady(@NonNull File pictureFile, boolean sinkFile, long sequenceCounter) {

		final Uri pictureUri = Utils.scanMediaFile(this, pictureFile);
	...
	@Override
	public void onDetectCorners(CameraView inst, PointF[] points, int failure, float failedRate, float areaAspect) {
		if (failure == FindDocCornersThread.CORNERS_DETECTED) {
			...
			switch (failure) {
				...
				case FindDocCornersThread.CORNERS_UNCERTAIN:
					...
					break;
				default:
					text = SdkFactory.verboseDetectionFailure(getApplicationContext(), failure, failedRate, areaAspect);
			}
